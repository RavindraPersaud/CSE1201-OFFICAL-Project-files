...
int main() {
    ...
    char target_word[MAX_MESSAGE_LENGTH];

    printf("Enter a message: ");
    fgets(message, sizeof(message), stdin);
    message[strcspn(message, "\n")] = '\0'; // remove newline if any

    printf("Enter the word to redact: ");
    scanf("%s", target_word);

    char* redacted = redact_message(message, target_word);

    if (redacted != NULL) {
        printf("\nOriginal Message: %s\n", message);
        printf("Redacted Message: %s\n", redacted);
        free(redacted); // Don't forget to free the memory
    } else {
